...
time_delay = 2; %each frequency will show 'time_delay', sec, we record 100ms/2000ms (the value should be 0.1/2) data for each frequency band
...
for i = n_first:n_last
    ...
    figure
    [pxx0,f0] = pspectrum(minus_mean_sound_data,fs, 'Leakage',leakage_value,'FrequencyLimits',[low_fre high_fre]);    
    plot(f0,pow2db(pxx0))
...
% get the data without input frequency band
    ...
    figure
    [pxx1,f1] = pspectrum(minus_mean_sound_data_filter,fs, 'Leakage',leakage_value,'FrequencyLimits',[low_fre high_fre]);    
    plot(f1,pow2db(pxx1))
